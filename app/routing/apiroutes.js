// dependencies
var friendData = require('../data/friends.js');

// exporting friend matching function
module.exports = function (app) {

	// fetching data from existing friends
	app.get('/api/friends', function(req, res){
		res.json(friendData);
	})

	// yay! posting new friend data and matching!
	app.post('/api/friends', function(req, res){
		var newFriend = req.body;

		for(var i = 0; i < newFriend.userScores.length; i++) {
			if(newFriend.userScores[i] == "1") {
				newFriend.userScores[i] = 1;
			} else if(newFriend.userScores[i] == "5") {
				newFriend.userScores[i] = 5;
			} else {
				newFriend.userScores[i] = parseInt(newFriend.userScores[i]);
			}
		}

		var differencesArray = [];

		for(var i = 0; i < friendData.length; i++) {

			var comparedFriend = friendData[i];
			var totalDifference = 0;
			
			for(var k = 0; k < comparedFriend.scores.length; k++) {
				var differenceOneScore = Math.abs(comparedFriend.userScores[k] - newFriend.userScores[k]);
				totalDifference += differenceOneScore;
			}

			differencesArray[i] = totalDifference;
		}

		var bestFriendNum = differencesArray[0];
		var bestFriendIndex = 0;

		for(var i = 1; i < differencesArray.length; i++) {
			if(differencesArray[i] < bestFriendNum) {
				bestFriendNum = differencesArray[i];
				bestFriendIndex = i;
			}
		}

		friendData.push(newFriend);

		res.json(friendData[bestFriendIndex]);
	})
}